...
test('check statusCode sended is correct', t => {
  t.plan(5)
  const fastify = Fastify()
  fastify.register(responseApi)

  fastify.register((instance, opts, next) => {
    instance.get('/test-responseApi2', (request, reply) => {
      reply.error({ statusCode: 403, message: 'Forbidden!' })
    })
  ...
  fastify.listen(0, err => {
    ...
    sget({
      method: 'GET',
      url: 'http://localhost:' + fastify.server.address().port + '/test-responseApi2'
    }, (err, response, body) => {
      t.error(err)
      t.strictEqual(response.statusCode, 403)
      t.strictEqual(response.headers['content-length'], '' + body.length)
      t.equal(JSON.parse(body).response.message, 'Forbidden!')
      t.end()
    })
    // t.equal(fastify.res.error().response.error, true)
  })
